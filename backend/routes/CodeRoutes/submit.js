...
router.post("/", async function (req, res, next) {
  ...
  const question = await db.Question.findOne({ Q_id: qid }).catch((e) =>
    next(e)
  );
  ...
  const answer = await db.Answer.findOne({ A_id: qid }).catch((e) => next(e));
  ...
  if (!isCorrect) {
    ...
    const givePoint = async () => {
      const user = await db.User.findOne({ email: useremail }).catch((e) =>
        next(e)
      );
